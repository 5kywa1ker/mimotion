...
- **步数递增保护**（本版本独有）：脚本会在 `encrypted_tokens.data` 中同时记录每个账号当天最后一次成功提交的步数，下次执行时随机下限不低于该值（同一天内生效，跨天自动重置），避免相邻两次执行步数不升反降、步数曲线出现回落。提交失败不会更新记录，不影响下次执行。
...
2. 步数随机范围随时间线性增长，北京时间 22 点达到 `MIN_STEP~MAX_STEP` 最大值：10 点时约为 `10/22 × MIN_STEP ~ 10/22 × MAX_STEP`。可自行修改 `MIN_STEP` / `MAX_STEP`。同一天内多次执行时，脚本会保证步数不低于上次成功提交的值（步数递增保护），跨天后重新按时间曲线随机。
